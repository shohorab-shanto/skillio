...
class SessionBookingApiController extends Controller
{
    ...
    /**
     * Create Payment Intent for session booking (Step 1 - New Flow)
     */
    public function createPaymentIntent(Request $request, SessionBooking $session)
    {
        try {
            $user = Auth::user();
    ...
    /**
     * Confirm session booking after payment (Step 2 - New Flow)
     */
    public function confirmBooking(Request $request, SessionBooking $session)
    {
        try {
            $user = Auth::user();
            ...
            if ($session->status != 'active' || $session->user_id) {
                return response()->json([
                    ...
                    'error' => 'Session was booked by another user or is not active'
                ], 400);
            }
            ...
            try {
                ...
                $grossAmount = $metadata->gross_amount ?? $session->fee;
