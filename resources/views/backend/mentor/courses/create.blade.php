            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="duration_type" name="duration_type" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent" required onchange="toggleDurationType()">
                        <option value="days" {{ old('duration_type') == 'days' ? 'selected' : '' }}>{{ __('trans.days') ?? 'Days' }}</option>
                        <option value="hours" {{ old('duration_type') == 'hours' ? 'selected' : '' }}>{{ __('trans.hours') ?? 'Hours' }}</option>
                    </select>
                    @error('duration_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
...
function updateSubCategories() {
    ...
    // Clear existing options and reset selected
            ...
            selectedCategory.sub_categories.forEach(subCat => {
                ...
                optionDiv.innerHTML = `
                    <div class="flex justify-between items-center w-full">
                        ...
                        <span class="hidden selected-check">
...
// Initialize sub-categories on page load if category is already selected
document.addEventListener('DOMContentLoaded', function() {
    const categorySelect = document.getElementById('category_id');
    if (categorySelect.value) {
        updateSubCategories();
        
        // Restore old sub-category selections
        const oldSubCategories = @json(old('sub_category_ids', []));
        if (oldSubCategories.length > 0) {
            setTimeout(() => {
                const subCategorySelect = document.getElementById('sub_category_ids');
                Array.from(subCategorySelect.options).forEach(option => {
                    if (oldSubCategories.includes(option.value)) {
