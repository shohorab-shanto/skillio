        ...
        <form method="POST" action="{{ route('mentor.time-slots.update', $timeSlot) }}" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="currency" name="currency" 
                        ...
                        <option value="USD" {{ old('currency', $timeSlot->currency) == 'USD' ? 'selected' : '' }}>USD ($)</option>
                        <option value="EUR" {{ old('currency', $timeSlot->currency) == 'EUR' ? 'selected' : '' }}>EUR (€)</option>
                    </select>
                    @error('currency')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <div class="relative max-w-xs">
                    <span id="currency-symbol" class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500">
                        {{ $timeSlot->currency == 'EUR' ? '€' : '$' }}
                    </span>
                    <input type="number" 
                           name="fee"
                           step="0.01"
